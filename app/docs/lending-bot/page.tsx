...
const t: Record<Lang, {
  ...
  en: {
    ...
    whatIsIt: "What is the Lending Bot?",
    whatIsItDesc: "The Lending Bot is an automation system built into Agio that executes lending operations on your behalf. It is not an AI agent — it is a deterministic bot that follows pre-configured rules. Each user can activate one bot instance with its own dedicated Solana wallet managed by Privy.",
    ...
    activationDesc: "You can activate the Lending Bot from the Agent tab in your dashboard. Once activated, fund the bot wallet with SOL (for transaction fees) and the tokens you want the bot to use. Configure your strategy parameters, and the bot will start operating on the next cron cycle.",
  },
  es: {
    title: "Bot de Préstamos",
    lead: "Un bot automatizado que gestiona posiciones de préstamos 24/7 según tu estrategia configurada.",
    whatIsIt: "Qué es el Bot de Préstamos?",
    whatIsItDesc: "El Bot de Préstamos es un sistema de automatización integrado en Agio que ejecuta operaciones de préstamos en tu nombre. No es un agente de IA — es un bot determinístico que sigue reglas preconfiguradas. Cada usuario puede activar una instancia del bot con su propia wallet Solana dedicada gestionada por Privy.",
    ...
    activationDesc: "Puedes activar el Bot de Préstamos desde la pestaña Agente en tu dashboard. Una vez activado, fondea la wallet del bot con SOL (para tarifas de transacción) y los tokens que quieras que use el bot. Configura los parámetros de tu estrategia y el bot comenzará a operar en el próximo ciclo cron.",
  },
  pt: {
    title: "Bot de Empréstimos",
    lead: "Um bot automatizado que gerencia posições de empréstimos 24/7 com base na sua estratégia configurada.",
    whatIsIt: "O que é o Bot de Empréstimos?",
    whatIsItDesc: "O Bot de Empréstimos é um sistema de automação integrado ao Agio que executa operações de empréstimos em seu nome. Não é um agente de IA — é um bot determinístico que segue regras pré-configuradas. Cada usuário pode ativar uma instância do bot com sua própria wallet Solana dedicada gerenciada pelo Privy.",
    ...
    activation: "Ativação",
    activationDesc: "Você pode ativar o Bot de Empréstimos na aba Agente do seu dashboard. Uma vez ativado, financie a wallet do bot com SOL (para taxas de transação) e os tokens que você quer que o bot use. Configure os parâmetros da sua estratégia e o bot começará a operar no próximo ciclo cron.",
  },
  zh: {
    title: "借贷机器人",
    lead: "一个自动化机器人，根据您配置的策略全天候管理借贷仓位。",
    whatIsIt: "什么是借贷机器人？",
    whatIsItDesc: "借贷机器人是 Agio 内置的自动化系统，代表您执行借贷操作。它不是 AI 代理——它是一个遵循预配置规则的确定性机器人。每个用户可以激活一个机器人实例，拥有由 Privy 管理的专用 Solana 钱包。",
    howItWorks: "工作原理",
    ...
    activation: "激活",
    activationDesc: "您可以从仪表板的代理标签页激活借贷机器人。激活后，用 SOL（用于交易费）和您希望机器人使用的代币为机器人钱包充值。配置您的策略参数，机器人将在下一个 cron 周期开始运行。",
  },
}
