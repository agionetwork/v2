...
pub struct CreateLendOfferArgs {
    pub debt_amount: u64,
...
#[derive(Accounts)]
#[instruction(args: CreateLendOfferArgs)]
...
pub fn create_lend_offer<'info>(
    ...
    if args.is_private && args.lender.is_none() {
        return Err(error!(AgioError::MissingPrivateLender));
    }

    if args.is_private {
        loan.lender = args.lender;
        loan.private_status = PrivateStatus::PrivateLender as u8;
    }
    ...
    if ctx.accounts.collateral_mint.key() == WSOL_MINT.key() {
        ...
        transfer(cpi_context, args.collateral_amount)?;
    } else {
