...
export function registerPaidTools(server: McpServer) {
  ...
  server.tool(
    ...
    "Update the lending/borrowing configuration for your Agio agent. Free — no payment required.",
    {
      ...
      lendMinCollateralRatio: z.number().min(150).max(500).optional().describe("Min collateral %, e.g. 150 (protocol minimum 150%)"),
      lendMaxCollateralRatio: z.number().min(150).max(500).optional().describe("Max collateral %, e.g. 300 (protocol minimum 150%)"),
      ...
      borrowMinCollateralRatio: z.number().min(150).max(500).optional().describe("Min collateral %, e.g. 150 (protocol minimum 150%)"),
      borrowMaxCollateralRatio: z.number().min(150).max(500).optional().describe("Max collateral %, e.g. 300 (protocol minimum 150%)"),
